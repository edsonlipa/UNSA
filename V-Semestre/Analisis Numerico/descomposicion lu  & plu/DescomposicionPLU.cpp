...
void print_PLU(matrix P,matrix L,matrix U) {
	cout<<"---P---"<<endl;
	print(P);
	cout<<"---L---"<<endl;
	print(L);
...
void upperTriangle(matrix &U,matrix &L,matrix &P)
{	//cout<<"triangulo superior"<<endl;
	numero multip=0;
	for (unsigned i = 0; i < U.size(); ++i)//columan
	{
		maxibyColmns(U,i,P,L);
		for (unsigned j = i+1; j < U.size(); ++j)//fila
		{
			if(U[j][i]!=0)
			{
			multip=(U[j][i]/U[i][i]);
			...
			U[j]=multandsubt(U[j],U[i],multip);
			//multbyRow(U[j],multip);
			//subtBetweenRow(U[j],U[i]);
			// std::cout << "proceso de triangular superior" << '\n';
			print_PLU(P,L,U);
...
int main(int argc, char const *argv[]) {
  // matrix A{{13,4,-1,1},{2,-9,1,-3},{1,2,-13,-1},{-3,1,1,7}};
	...
	matrix A{{2,3,1,-1},{3,-4,-1,1},{4,-6,10,-7},{1,-1,3,9}};
	colmnMatrix B{10,20,25,18};
	print(A);
	print(B);
  row ans=resolver_conPLU(A,B);
	// print(ans);
	// matrix A{{1,2,-2,1},{4,5,-7,6},{5,25,-15,-3},{6,-12,-6,22}};
	// matrix U;
	// matrix L(A.size(),row(A.size()));
	// matrix P;
	//
