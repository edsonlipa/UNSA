...
int main (){
    //vector<T_Arista> aristas;
    

    minheap aristas;
    int n_cities,n_roads,cost,n_cases;
    cin >> n_cities;
    cin >> n_roads;
    string from,to,case_from,case_to;
    for(size_t i=0 ; i<n_roads; i++){
        cin>>from>>to>>cost;
        aristas.push(make_tuple(from,cost,to));
    }
     
    cin >> n_cases;
    for(size_t i=0 ; i<n_cases; i++){
    DisjointSet DS(n_cities);
    cin>>case_from>>case_to;
    cout<<DS.mst(case_from,case_to,n_roads,aristas)<<endl;
    }
    return 0;   
}